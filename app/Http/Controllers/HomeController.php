...
class HomeController extends Controller
{
    ...
    {
        $page = DB::table('pages')->where('id', 1)->first();
        $search = urldecode(trim($request->input('search'))); // Decode search query
        $excludedIds = [464];

        // Initialize Query Builder
        $query = Product::query()->where('status', '1')->whereNotIn('id', $excludedIds);
        ...
        if (!empty($subcat)) {
            $query->where('subcategory', $subcat);
        }
        if (!empty($childsubcat)) {
            $query->where('childsubcategory', $childsubcat);
        }
        ...
        if (isset($custom_ordering_products[$cat])) {
            $orderIds = $custom_ordering_products[$cat];
            $orderString = implode(',', $orderIds);
            $query->orderByRaw("FIELD(id, $orderString) DESC");
        }
